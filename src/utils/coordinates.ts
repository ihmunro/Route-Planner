  ...
  if (matches.length > 0) {
    ...
    matches.forEach(match => {
      ...
      if (isNaN(lat) || isNaN(lng) || !validateCoordinate(lat, lng)) {
        console.warn(`Skipping invalid coordinate: ${match[0]}`);
      }
    ...
    for (let i = 0; i < pairs.length; i += 2) {
      ...
      if (isNaN(lat) || isNaN(lng) || !validateCoordinate(lat, lng)) {
        console.warn(`Skipping invalid coordinate at position ${i + 1}`);
      }
...
export const calculateDistance = (coord1: Coordinate, coord2: Coordinate): number => {
  ...
  const lat1 = toRad(Number(coord1.lat));
  const lat2 = toRad(Number(coord2.lat));
  const dLat = toRad(Number(coord2.lat) - Number(coord1.lat));
  const dLon = toRad(Number(coord2.lng) - Number(coord1.lng));
...
const getSpeedLimit = (distance: number): number => {
  // Very basic speed limit assignment based on distance
  if (distance < 1) {
    return 30; // City streets (km/h)
  } else if (distance < 10) {
    return 60; // Suburban roads (km/h)
  } else {
    return 100; // Highways (km/h)
  }
};

export const calculateTime = (distance: number): number => {
  const averageSpeed = getSpeedLimit(distance);
